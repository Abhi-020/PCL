...
def main(args):
    ...
    nclasses = 2#len(list(df_final.label_.unique()))
    ...
    print(y[:,1].shape)
    train_dataset, val_dataset, y_train, y_val = train_test_split(X_data,  y[:,args.classnum], test_size = 0.20, random_state= 0)
    ...
    f =open(args.loglocation + f'{args.lm}_test_class{args.classnum}_task2_1.txt', 'w')
    for i in y_pred:
      print(i, file = f )
    f.close()
    print('file saved!!!!')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classnum', required=True, type=int,
                        help='Pick a class from 0 to 6 ')
